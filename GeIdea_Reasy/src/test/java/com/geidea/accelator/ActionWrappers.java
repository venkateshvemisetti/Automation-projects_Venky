...
public class ActionWrappers {
	...
	public static void waitUntilSpinnerDisappers() {
		try {
			...
			boolean flag = true;
			// System.err.println("spinner name :
			// "+driver.findElement(By.xpath("//div[@class='x-mask-msg']/div")).getText());
			while (flag == true) {
				List<WebElement> element = driver.findElements(By.xpath(
						"//div[@class='x-mask-msg']/div | //div[@id='mask' and contains(@style,'display: block;')]"));
				if (element.size() > 0) {
	...
	public void mouseOverToElement(WebElement element) {
		try {
			...
			// extentTest.log(LogStatus.INFO, "Mouse hover to the Element");
		} catch (Exception e) {
	...
	public void selectValFromSelectCumTextBox(String xpathForSelectBoxKey, String xpathForSelectInputBoxKey,
			String reqOptionOrText) {
		try {
			String dropDownOptions = xpathForSelectBoxKey
					+ "//*/following::div[@class='dropdown-menu show']//div[@class='dropdown-list']/a | "+xpathForSelectBoxKey+"//*/following::div[@class='dropdown-menu show']//div[@class='dropdown-list withPagination']/a";
			int dropDownOptionsCount = 0;
			By s1 = By.xpath(xpathForSelectBoxKey);
			List<WebElement> xpathForSelectBox = findElementts(s1);
			if (xpathForSelectBox.size() > 0) {
				List<WebElement> xpathForSelectBox1 = findElementts(s1);
				mouseOverToElement(xpathForSelectBox1.get(0));
				if (xpathForSelectBox.get(0).isDisplayed()) {
					xpathForSelectBox.get(0).click();
					// clickElementUsingActions(xpathForSelectBox.get(0));
					// TestUtilsdriver.manage().timeouts().implicitlyWait(30,
					// TimeUnit.SECONDS);1800);
					// driver.manage().timeouts().implicitlyWait(1, TimeUnit.SECONDS);
					By s2 = By.xpath(dropDownOptions);
					List<WebElement> dropDownOptionsList = findElementts(s2);
					if (dropDownOptionsList.size() > 0) {
						dropDownOptionsCount = dropDownOptionsList.size();

						if (dropDownOptionsCount != 0) {
							By s3 = By.xpath(xpathForSelectInputBoxKey);
							List<WebElement> xpathForSelectInputBox = findElementts(s3);
							if (xpathForSelectInputBox.size() > 0) {
								extentTest.info("select box is clicked. options are displayed successfully");
								if (xpathForSelectInputBox.get(0).isDisplayed()) {

									if (xpathForSelectInputBox.get(0).getAttribute("value").equals("")
											|| xpathForSelectInputBox.get(0).getAttribute("value").equals(" ")) {
										xpathForSelectInputBox.get(0).sendKeys(reqOptionOrText);
										List<WebElement> accountTypeSelectInputBoxSuccessMsg = driver
												.findElements(By.xpath(dropDownOptions));
										if (accountTypeSelectInputBoxSuccessMsg.size() > 0) {
											extentTest.info("Entered required text is available in options");
											...
											if (xpathForSelectBox.get(0).getText().trim().equals(reqOptionOrText)) {
												extentTest.info("Required option <b> " + reqOptionOrText
														+ "</b> is selected successfully");
												extentTest.info("Snapshot below: " + takeScreenShot());
											}
										} else {
											extentTest.error(
													"Entered required text or option is not available in options");
											extentTest.info("Snapshot below: " + takeScreenShot());
										}
									} else {
										extentTest.warning("Some text is available in input box");
										extentTest.info("Snapshot below: " + takeScreenShot());
										xpathForSelectInputBox.get(0).clear();
										if (xpathForSelectInputBox.get(0).equals("")
												|| xpathForSelectInputBox.get(0).equals(" ")) {
											extentTest.info("Select input box is cleared");
											...
											if (accountTypeSelectInputBoxSuccessMsg.size() > 0) {
												extentTest
												.info("Entered required account type is available in options");
												Robot robot = new Robot();
												robot.keyPress(KeyEvent.VK_TAB);
												robot.keyRelease(KeyEvent.VK_TAB);
												robot.keyPress(KeyEvent.VK_TAB);
												...
												robot.keyPress(KeyEvent.VK_ENTER);
												robot.keyRelease(KeyEvent.VK_ENTER);
												// xpathForSelectInputBox.get(0).sendKeys(Keys.ENTER);

												if (xpathForSelectBox.get(0).getText().trim().equals(reqOptionOrText))
													extentTest.info("Required option is selected successfully");
											} else {
												extentTest.error(
														"Entered required text or option is not available in options");
												extentTest.info("Snapshot below: " + takeScreenShot());
									...
									extentTest.error("Select input text box is not displayed or not enabled");
									extentTest.info("Snapshot below: " + takeScreenShot());
								}

							} else {
								extentTest.error("Select input text box is not loaded in page");
								extentTest.info("Snapshot below: " + takeScreenShot());
							}
						} else {
							int count = 0;
							for (WebElement eachDropDownOption : dropDownOptionsList) {
								if (eachDropDownOption.isDisplayed()) {
									if (eachDropDownOption.getText().trim().equals(reqOptionOrText)) {
										eachDropDownOption.click();
										driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
										if (xpathForSelectBox.get(0).getText().trim()
												.equalsIgnoreCase(reqOptionOrText.trim()))
											extentTest.info("Required option <b>" + reqOptionOrText
													+ "</b> is selected successfully");
										break;
									}
						...
						extentTest.error("Dropdown options are not loaded in page");
						extentTest.info("Snapshot below: " + takeScreenShot());
					}

				} else {
					extentTest.error("select box is not displayed");
					extentTest.info("Snapshot below: " + takeScreenShot());
				}
			} else {
				extentTest.error("Select box is not loaded in a page");
				extentTest.info("Snapshot below: " + takeScreenShot());
			}
		} catch (Exception e) {
			extentTest.error(e);
	...
	public void uploadFile(By uploadoptionbts,By fileoption, String filepath ) {
		try {
			System.out.println("uploadFile ::: method");
			List<WebElement> uploadoptionbt=driver.findElements(uploadoptionbts);
			if (uploadoptionbt.size()>0) {
				if (uploadoptionbt.get(0).isDisplayed() && uploadoptionbt.get(0).isEnabled()) {
					uploadoptionbt.get(0).click();
					List<WebElement> browesefile=driver.findElements(fileoption);
					if (browesefile.size()>0) {
						/*
						 * extentTest.pass("file uplaod options are loaded");
						 * extentTest.info(takeScreenShot());
						 */
						System.out.println("file uplaod options are loaded");
						uploadFile(fileoption, filepath);
						
					}
					else
					{
						System.out.println("file uplaod options are not loaded");
						/*
						 * extentTest.fail("file uplaod options are not loaded");
						 * extentTest.info(takeScreenShot());
						 */
					}
				...
				{
					//extentTest.error("uploadoptionbt is not enabled or displayed");	
					System.out.println("uploadoptionbt is not enabled or displayed");
				}
				
			}
			else
			{
				//extentTest.error("uploadoptionbt size not macthed");
				System.out.println("uploadoptionbt size not macthed");
			}
	...
	public void uploadFile(By fileoption, String filepath ) {
		try {
			System.out.println("uploadFile :: method");
			List<WebElement> browesefile=driver.findElements(fileoption);
			
			if (browesefile.size()>0) {
				if (browesefile.get(0).isDisplayed() || browesefile.get(0).isEnabled()) {
				  clickUsingJSE(browesefile.get(0));
					...
					//extentTest.error("browesefile is not enabled or displayed");
					System.out.println("browesefile is not enabled or displayed");
				}

			} else {
				extentTest.error("browesefile size not macthed");
				System.out.println("browesefile size not macthed");
