...
import com.rjio.objects.LogoutObject;
		...
		try {
			if (findElementts(LogoutObject.logoutBTN).size() > 0) {
				if (findElementt(LogoutObject.logoutBTN).isEnabled()) {
					...
					sleep(1500);
					extentTest.info("Post logging-out the Application", MediaEntityBuilder.createScreenCaptureFromPath(takeScreenShot()).build());
				} else {
