...
public class AlertsAccountsPage extends ActionWrappers {
			...
			if (findElementts(AlertsAccountsObject.searchResult).size() > 0) {
				...
				if (findElementts(AlertsAccountsObject.editAccountIcon).size() == 1) {
					if (findElementt(AlertsAccountsObject.editAccountIcon).isDisplayed()
						...
						extentTest.warning("editAccountIcon is not displayed/enabled");
					}
				} else {
					extentTest.warning("editAccountIcon size not matched");
	...
	public void searchAccount(String name) {
		try {
			System.out.println("searchAccount : "+name);
				...
				if (findElementts(AlertsAccountsObject.stopAccountIcon).size() == 1) {
					if (findElementt(AlertsAccountsObject.stopAccountIcon).getAttribute("data-qtip").contains("Stop")) {
						...
						if (findElementts(AlertsAccountsObject.searchResult).size() > 0) {
							...
							extentTest.warning("searchResult size not macthed :: post stopping");
					...
					extentTest.warning("stopAccountIcon size not matched");
				}
		...
		try {
			...
			if (findElementts(AlertsAccountsObject.searchResult).size() > 0) {
				...
				if (findElementts(AlertsAccountsObject.startAccountIcon).size() == 1) {
					...
					extentTest.warning("startAccountIcon size not matched");
	...
	public void deleteAccount(JsonNode node) {
		try {
			...
			waitUntilSpinnerDisappers();
			moveFrame("bodyframe");
			List<WebElement> searchAccNames=findElementts(AlertsAccountsObject.searchAccName);
			if (findElementts(AlertsAccountsObject.searchResult).size() == 1 && searchAccNames.size()==1) {
				String searchAccName = searchAccNames.get(0).getText();
				extentTest.info("The found Account is : " + searchAccName);
				if (accountName.equals(searchAccName)) {
					if (findElementts(AlertsAccountsObject.deleteAccountIcon).size() == 1) {
						...
						extentTest.fail("deleteAccountIcon size not matched");
						System.err.println("deleteAccountIcon size not matched");
					}
				} else {
					System.err.println("mismacthed accounts " + accountName + " , " + searchAccName);
					extentTest.warning("mismacthed accounts " + accountName + " , " + searchAccName);
				}
			} else {
				System.err
				.println("no records found : deletion Account :: searchResult/searchAccNames size not matched " + accountName);
				extentTest.warning("no records found : deletion Account :: searchResult/searchAccNames size not matched : " + accountName);
			}
			clear(AlertsAccountsObject.searchfiled, "searchfiled");
			backToFrame();
			waitUntilSpinnerDisappers();
